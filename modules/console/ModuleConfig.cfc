...
<cfscript>
	...
	function configure(){
		...
		// Model Bindings
		binder.map("DSLService@console").to("#moduleMapping#.model.DSLService");
		binder.map("Relaxer@console").to("#moduleMapping#.model.Relaxer");
		binder.map("DSLDoc@console").to("#moduleMapping#.model.DSLDoc");
		// RelaxLogs Bindings
		/*
		binder.map("logService@consolelogs").to("#moduleMapping#.model.logbox.LogService");
		binder.map("MSSQL_DAO@consolelogs").to("#moduleMapping#.model.logbox.MSSQL_DAO");
		binder.map("MYSQL_DAO@consolelogs").to("#moduleMapping#.model.logbox.MYSQL_DAO");
		binder.map("ORACLE_DAO@consolelogs").to("#moduleMapping#.model.logbox.ORACLE_DAO");
		binder.map("POSTGRES_DAO@consolelogs").to("#moduleMapping#.model.logbox.POSTGRES_DAO");
		*/
	...
	*/
	function preProcess(event,interceptData) eventPattern="^console.*"{
	...
	function loadDefaultAPI(){
		var DSLService = controller.getWireBox().getInstance("DSLService@console");
